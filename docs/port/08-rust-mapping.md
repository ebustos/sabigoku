...
| 2026-07-26 | ROD-497: sync progress stops being raise-only. `reconcile` runs the three-way matrix on the progress half as well as the status half, so a remote-only move is adopted downward; the duplicate-entry collapse switches from max-progress to latest `updatedAt` (query widened) because collapsing by magnitude re-raises exactly what the merge exists to lower; the merged pair snaps to total when the status half lands on `completed`, holding the `setListStatus` invariant the two independent matrices could otherwise break. Root cause: zigoku's unconditional `max` cannot represent a correction, and re-baselining the snapshot to the lower remote then queues the stale local value to overwrite it on the server. Follow-on to ROD-477: the frontier can now move backward, so `progress_stamped_at` stamps on a RISE only (05 §10.7, DESIGN 4.6 amended); `recompute` keeps stamping both ways by design. DESIGN 4.5 gains a watched-past-broadcast register instead of capping the bar fill at the aired count, which would hide a real watch for a full enrichment TTL |
