...
| 2026-07-26 | ROD-465: boot update-check lands as `src/updatecheck.rs` + `src/semver.rs` (06 §6.1), the zigoku pair ported semantics-intact (two-line cache, future-dated = stale, silent failure, coarse prerelease ordering). `UpdateAvailable` re-added to the event enum (the a125603 back-out was the wiring map); the worker rides `sync_drain`, gated on `check_for_updates` in `bootstrap_update_check`. Settings gains the Updates section (DESIGN 5.5 amended: inert `version` row + `check for updates` toggle, boundaries `13..14`). DEVIATIONS, both ratified: 06 §6.1's TTL said 6 hours but zigoku's frozen source says 1 hour with a rate-limit rationale; transcription drift, doc corrected to 1h. Tag sanitization hoisted from zigoku's CLI print into `parse_latest_tag`, so the TUI toast path is covered by the same strip |
