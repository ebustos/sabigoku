...
| 2026-07-25 | ROD-491: the CLI query path binds to a SEARCH-CAPABLE provider, not merely the preferred one. RATIFIED DEVIATION from 03 §3.2 and 06 §7.3. zigoku at freeze (main.zig:178, "CLI does not walk the registry") takes `registry.preferred(cfg.preferred_provider)` and hard-fails when that provider cannot search; its own test asserts `preferred("")` is megaplay, and 03 §8.1 says megaplay has no tier C. So a STOCK zigoku install has a dead `zigoku <query>` too: this was faithful parity with an upstream defect, not port drift, and Rod's live zigoku only works because his config pins a searchable source. New `StreamProvider::supports_search()` (default true, false in megaplay) makes the structural fact structural instead of discovered by attempting; `ProviderRegistry::preferred_searchable(pref)` returns the first `ordered()` entry that can search. SCOPE OF THE WALK: search only. Whichever provider answers owns the whole run, because a provider id is meaningless on another (03 §3.2 binding rule is untouched). An explicit but incapable preference is walked past with a one-line note naming both. The old Unsupported/Search copy pointed users at `preferred_provider`; that advice now describes an unreachable state, so it is gone and the test asserting it inverted (`search_unsupported_no_longer_nudges_at_config`). TUI unaffected: it already walked via `ordered()`. Guard mutation-checked: replacing the `supports_search` filter with `.next()` fails 3 of the 4 new registry tests |
| 2026-07-26 | ROD-497: sync progress stops being raise-only. `reconcile` runs the three-way matrix on the progress half as well as the status half, so a remote-only move is adopted downward. The duplicate-entry collapse moves from max-progress to `(updatedAt, progress)`, query widened for the stamp: max-progress was a sabigoku invention citing a 06 clause that never existed, and it re-raised exactly what the merge exists to lower; zigoku dedupes last-wins by wire order (`anilist.zig:605-611`) and never fetched `updatedAt`. The merge takes NO status special-case on progress. Four rounds of guarding the completed/partial synthesis each collapsed to overriding the remote-lowered-progress cell that ROD-497 exists to preserve, so the synthesis is accepted; the pair is legal on AniList. The first attempt raised to `total_episodes` and was rejected for minting watch data on settled rows, pushing it, and re-minting every pull. Root cause: zigoku's unconditional `max` cannot represent a correction, and re-baselining the snapshot to the lower remote then queues the stale local value to overwrite it on the server. Follow-on to ROD-477: the frontier can now move backward, so `progress_stamped_at` stamps on a RISE only (05 §10.7, DESIGN 4.6 amended); `recompute` keeps stamping both ways by design. DESIGN 4.5 gains a watched-past-broadcast register, unlit so the bright run stops at the broadcast edge, instead of capping the bar fill, which would hide a real watch for a full enrichment TTL; deliberately NOT reconciled with the §4.6 grid cap (02 §4), since a grid cell is an affordance and the bar is a record |
| 2026-07-26 | ROD-465: boot update-check lands as `src/updatecheck.rs` + `src/semver.rs` (06 §6.1), the zigoku pair ported semantics-intact (two-line cache, future-dated = stale, silent failure, coarse prerelease ordering). `UpdateAvailable` re-added to the event enum (the a125603 back-out was the wiring map); the worker rides `sync_drain`, gated on `check_for_updates` in `bootstrap_update_check`. Settings gains the Updates section (DESIGN 5.5 amended: inert `version` row + `check for updates` toggle, boundaries `13..14`). DEVIATIONS, both ratified: 06 §6.1's TTL said 6 hours but zigoku's frozen source says 1 hour with a rate-limit rationale; transcription drift, doc corrected to 1h. Tag sanitization hoisted from zigoku's CLI print into `parse_latest_tag`, so the TUI toast path is covered by the same strip. REVIEW HARDENING (same day): `is_fresh` uses `saturating_sub` after a chaos repro showed a cache `checked_at` of i64::MIN reads as permanently fresh in a release build; the sanitizer centralized into `sanitize_tag` so `parse_cache` guards the cache file like the network body (the file is untrusted input and the warm path runs most boots); cache write is tmp+rename so a concurrent or abandoned write can never be read torn; fetch body capped at 64KB against a spoofed api.github.com. THIRD DEVIATION, ratified: zigoku JOINS its update thread at quit; sabigoku abandons it on a dedicated never-drained `update_drain`, so quit never waits on a nag (safe by the deadline cap + atomic write). `latest_fresh` cut until ROD-466 wires and tests it. semver keeps the spec-correct read of a hyphen inside build metadata (`0.4.1+build-info` parses; zigoku rejects it), pinned by test |
| 2026-07-26 | ROD-466: `sabigoku update` lands as `src/update.rs`, the update.zig port with a fourth channel arm: cargo (path check against `$CARGO_HOME/bin`, else `~/.cargo/bin`; zigoku has pacman/brew/standalone only). `decide()` stays the pure policy core, package ownership before writability. Detection order: pacman `-Qo` -> brew (`brew list` + prefix-boundary vs `brew --prefix`) -> cargo -> standalone. Standalone drives install.sh pinned to the freshly-fetched tag (env only, never shell-interpolated); `latest_fresh` restored with live coverage (tests/update_live.rs, ignored). DEVIATIONS: `dir_writable` is a probe write, not zigoku's `access(W_OK)` (ACLs and root-squash make permission bits lie); exe path via `std::env::current_exe` instead of per-OS syscalls; and TWO TEST SEAMS in prod code (`SABIGOKU_UPDATE_URL`, `SABIGOKU_INSTALL_URL`), because without them the default suite either reaches GitHub or, offline, falls through standalone detection and EXECUTES the real installer over the binary under test (the old stub test did exactly that once the stub became real). REVIEW HARDENING (three-way panel): the seams are now behind a default-off `test-seams` Cargo FEATURE, not raw env reads: `#[cfg(debug_assertions)]` was rejected because the AUR PKGBUILD runs `cargo test --release`, which would strip the seam mid-package-build, so CI passes `--features test-seams` and the CLI update tests are gated on it. `perform_update` takes a confirmed-newer, safe_ref-validated tag (gates BOTH the URL ref AND SABIGOKU_VERSION in this layer, not delegated to install.sh's charset gate); the master-fallback is gone. `run` no longer self-updates when the freshness check fails (review: a rate-limited API call would curl|sh an unpinned reinstall over a current binary); package-manager directions still print (inert). Installer child now has a 300s wall-clock timeout (a stalled fetch wedged `update` forever); it runs in its OWN process group (`process_group(0)`) and the timeout kills the whole group via `kill -KILL -pgid` (kill(1), not libc; the crate ships no unsafe), because a bare `child.kill()` reaped only the `sh -c` wrapper and left install.sh's own subtree orphaned to init, still running after the tool reported the binary untouched (review re-attack, MEDIUM). cargo detection gained CARGO_INSTALL_ROOT precedence (the `install.root` config key stays UNREAD, a documented gap). detect_method's PATH-order trust is an accepted footgun (misdetection only changes advice, never mints a bad install). MINOR NAMED DEVIATIONS from update.zig: AUR help lists paru before yay, refusal curl uses -fsSL (matches install.sh's own header) |
| 2026-08-01 | ROD-525: the two-tier provider preference (global `preferred_provider` + per-show pin) retires for sticky last-used + `v` as a manual walk. Store V3: `provider_last_used` seeded from pins only where a binding exists, `provider_pin`/`provider_route` dropped, zimport gated on landed binding rows. Engine: `WalkOrigin::{Manual, Auto}` replaces the 3-tag set; one exhaust (`DeadEnd`); `route_preferred`, `open_history` (confirmed dead code, never called: the 03 §4.1 Path 2 law was never wired, drift recorded in the chapter), K-2, and `PinKept` deleted; manual walks wrap the registry circle from the aim and probe fresh absence on the first hop only. The landing in `land()` is the sole walk retirement point (fixes the armed-walk leak that ate play fail-overs and wedged prewarm) and the sole last-used writer, clearing on the walk-order head. UI: the §5.3a register is the selection cursor (armed aim, then in-flight probe); manual hops stay off the toast rail. The K-2 ledger row closes as moot; the false-green Path 2 contracts row is retired |
