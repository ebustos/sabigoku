#!/usr/bin/env bash
# capture-kitty.sh: README media runner (ROD-464, ported from zigoku ROD-255).
# Interprets a declarative .kbeats file to capture README media (stills + gifs)
# from the REAL sabigoku, including Kitty-graphics cover art that every
# cell-grid recorder (vhs, asciinema, agg) physically cannot render.
#
# Why kitty-under-Xvfb: the Kitty graphics protocol ships cover art as PIXELS to
# the framebuffer, not as text cells. Cell-grid recorders render covers as the
# halfblock fallback. The only capture that works is grabbing pixels from a
# terminal that implements the protocol, so we drive a real kitty inside a
# headless X server and screenshot / x11grab its framebuffer.
#
# Non-obvious recipe bits (each cost real debugging; do not "simplify" away):
#   1. kitty needs `-o linux_display_server=x11` (+ KITTY_ENABLE_WAYLAND=0).
#      Without it kitty starts, runs its child, stays alive, but never presents
#      its GL surface to the Xvfb framebuffer, so every grab is blank.
#   2. `unset TMUX` FIRST. If TMUX leaks in, the graphics protocol goes out in
#      tmux-passthrough form, which a non-tmux kitty rejects: no cover.
#   3. Software GL (Xvfb + llvmpipe) renders the protocol fine, incl. 15fps gifs.
#
# Reuses docs/media/capture-launch.sh VERBATIM (stubbed mpv, real store, real
# cover cache), inheriting its watchlist-READ-ONLY safety. On top, this runner
# LINTS every .kbeats file before it touches the store and refuses to run on:
#   - a store-writing key: P (plan), p/x/c/w (status), r (recompute), u (undo),
#     v (provider pin), X (arms hard-delete);
...
# What the lint does NOT cover (author discipline still required):
#   - Enter-to-PLAY: `Return` is safe as open/confirm in most views but PLAYS in
#     an episode grid. That path writes nothing only because capture-launch.sh
#     stubs mpv (no IPC socket, so no position update, so no store write); do
#     NOT send a `Return` inside an open episode grid anyway.
#   - a `type` beat outside an open `/`…search span (its chars reach the app as
#     commands).
#
# Usage:  ./docs/media/capture-kitty.sh [--check] docs/media/demo.kbeats
#   --check : lint the beats file ONLY (exit 0 = safe, 3 = flagged) and do NOT
#             boot Xvfb/kitty or touch the store.
# Requires: Xvfb, kitty, xdotool, xdpyinfo, ImageMagick (import + magick),
#           ffmpeg, gifski.
set -uo pipefail
...
if [ -z "$CHECK" ]; then
  ...
  [ -x "$ROOT/target/release/sabigoku" ] || { echo "no binary; run 'cargo build --release' first" >&2; exit 1; }
fi
...
#   - Safety: the Settings tour cycles the palette and saving would otherwise
#     write your real ~/.config/sabigoku/config.toml; here the save lands in $TMP.
#   - Sync off: no auth.toml means no AniList token, so background sync never
#     starts and nothing is pushed to your AniList list during a capture.
#   - Determinism: media renders in the default palette regardless of your
#     personal config, so the themes tour always matches the captions.
# Data store and cover cache stay REAL (XDG_DATA_HOME / XDG_CACHE_HOME untouched):
# the watchlist is yours and covers paint from cache without refetching.
export XDG_CONFIG_HOME="$TMP/config"
mkdir -p "$XDG_CONFIG_HOME/sabigoku"
# Seed two keys on top of the defaults (partial TOML is fine, Config is
# #[serde(default)]):
#   - landing "history": the default is last_watched, which auto-opens the
#     last-played show's detail on boot; beats need the plain History list.
#   - update check off: today it is silent (current == latest), but the first
#     re-capture after a release would stamp an "update available" toast into
#     the hero gif.
cat > "$XDG_CONFIG_HOME/sabigoku/config.toml" <<'TOML'
landing = "history"
...
for _ in $(seq 1 30); do
  WID=$(xdotool search --class sabicap 2>/dev/null | head -1)
  [ -n "$WID" ] && break
...
resize_win() {  # resize <W>x<H>: resize the window, let the app reflow, update geometry.
  # Lets one beats file capture wide views (Discover grid, two-pane detail) AND
  # narrow ones (single-pane detail, settings list) without dead space. NEVER call
  # mid-record: the x11grab region is fixed at record start. NB: kitty snaps to
  # cell-exact sizes.
  local rw="${1%x*}" rh="${1#*x}"
...
grab() {  # grab <file>: framebuffer cropped to the kitty window, kept under budget
  ...
  # Cover-heavy grabs (the Discover wall) can blow the still budget. A 256-colour
  # PNG fixes it with no visible banding and no downscale; only applied when over
  # budget. NB: du under-reports on a compressed FS (btrfs/zstd); measure logical
  # size with stat.
  bytes=$(stat -c%s "$out")
...
rec_start() {  # record <file> [fps=N] [width=N]
  ...
  # so the beats file is safe; this stops ffmpeg from fighting the invoking
  # terminal for it.
