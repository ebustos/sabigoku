...
How the README's screenshots and demos are produced and regenerated. **One
pipeline:** a real **kitty** driven inside a headless **Xvfb**, screenshotted /
recorded at the pixel level. Ported from zigoku's ROD-255 runner (ROD-464).

The Kitty graphics protocol ships cover art as **pixels** to the framebuffer,
not as text cells. Every cell-grid recorder (vhs, asciinema, termtosvg, agg)
models the terminal as a grid of cells, so it renders covers as the halfblock
fallback and can never capture one. Driving a real kitty and grabbing its
...
```sh
cargo build --release                            # beats drive ./target/release/sabigoku
...
> search, so the exact shows and covers vary per run. If a live search or feed
> is slow, a tour may catch a `searching…`/loading frame; re-run or lengthen the
> sleep. Timing is tunable per beats file.

## The `.kbeats` format

A beats file is a line-oriented capture script the runner interprets against a
live sabigoku. Comments start with `#`. Commands:
...
| `history.png` | `covers.kbeats` | Real watchlist master-detail, scrolled to a cover further down the groups. |

**On the hero's ending:** the play beat can't be shown headlessly (mpv is
stubbed), so playback lives in the caption + the status-bar affordance, and the
hero ends on the enriched detail. Deliberate, not missing.

## Safety (the live watchlist is never touched)

The runner reuses `capture-launch.sh` verbatim (stubbed mpv, real store) and
adds three guards on top:

- **Read-only lint.** The runner refuses to run any beats file containing a
  store-writing key: `P` (plan), `p/x/c/w` (status), `r` (recompute), `u`
  (undo), `v` (provider pin), `X` (arms delete); a single-char `type` command
  (`type p`); or a `grab`/`record` path that isn't a basename. Vet a beats file
  safely with `capture-kitty.sh --check <file>` (lints only, never boots Xvfb
  or touches the store). **Caveat:** a real run drives the REAL store, and the
  lint can't catch a multi-char `type` sent *outside* an open `/` search (its
  chars reach the app as commands) or a `Return` while a detail is focused
  (plays, and playing a show adds it to the watchlist). There is no store-level
  read-only mode (`Store::open` probes writable and fails loud on a read-only
  DB), so author discipline carries the rest: `--check`, keep every `type`
  inside a `/`…search span, and never send `Return` past the first drill-in.
- **Config isolation.** `XDG_CONFIG_HOME` points at a throwaway seeded with
  `landing = "history"` (the `last_watched` default would auto-open the
  last-played detail on boot) and `check_for_updates = false` (a re-capture
  after a release would otherwise stamp an "update available" toast into the
  hero). No `auth.toml` in the throwaway means no AniList token, so background
  sync never starts and nothing is pushed to your AniList list. The Settings
  tour's palette save is discarded with the dir, and the app renders in the
  **default** palette (deterministic media, independent of your personal
  config).
- **Real data + cache, read-only by construction.** `XDG_DATA_HOME` and
  `XDG_CACHE_HOME` are left real, so covers/watchlist are your actual library
  and art paints from cache; the beats are linted to never write to either.

## Tuning notes (verified in the zigoku pipeline, hold here)

- **kitty must present under Xvfb:** the runner sets `-o
  linux_display_server=x11` + `KITTY_ENABLE_WAYLAND=0`. Without it kitty runs
  but never paints to the framebuffer.
