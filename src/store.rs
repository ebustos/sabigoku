...
impl Store {
    ...
    /// is still not a play (02 §4b). The NULL-to-member transition seeds
    /// synced_* to the current pair (same born-clean hazard as the import
    /// mint): without it, list_dirty_for_sync pushes Planning/0 to AniList
    /// for a show mid-episode. A real status writer dirties it properly.
    pub fn record_engagement(&self, anilist_id: i64, now: i64) -> Result<(), Error> {
        self.conn.execute(
            "UPDATE show SET
                last_watched_at = :now,
                synced_status = CASE
                    WHEN library_added_at IS NULL AND synced_status IS NULL
                    THEN list_status ELSE synced_status END,
                synced_progress = CASE
                    WHEN library_added_at IS NULL AND synced_status IS NULL
                    THEN progress ELSE synced_progress END,
                library_added_at = COALESCE(library_added_at, :now)
             WHERE anilist_id = :id",
    ...
    /// the watch, not the recovery. Adopted rows are born clean like the
    /// engagement path above.
        ...
        let adopted = self.conn.execute(
            "UPDATE show SET
                ...
                last_watched_at =
                    (SELECT MAX(ep.updated_at) FROM episode_progress ep
                     WHERE ep.anilist_id = show.anilist_id),
                synced_status = CASE WHEN synced_status IS NULL
                    THEN list_status ELSE synced_status END,
                synced_progress = CASE WHEN synced_status IS NULL
                    THEN progress ELSE synced_progress END
...
impl Store {
    ...
    fn reconcile_plan(
        ...
        for e in remote {
            remote_map
                ...
                .or_insert((e.status, e.progress, e.updated_at));
        ...
        for (&id, &(status, progress, _)) in &remote_map {
    ...
    fn apply_reconcile(
        ...
        // partial, so it is dead" (05 §10.7); a downward adopt moves the
        // frontier behind the partial, which cannot retire it (ROD-497).
        for p in plan {
...
mod tests {
    ...
    fn orphaned_progress_is_adopted_into_history_at_open() {
        ...
        store
            ...
            .unwrap();
        store
            .save_progress(81, Translation::Sub, "2", 60.0, 1400.0, None, 700)
            .unwrap();
        ...
        // Ep "1" upserted twice survives at 530; membership takes the MIN,
        // last_watched the MAX, so the two must differ here.
        assert_eq!(ghost.library_added_at, Some(530));
        assert_eq!(ghost.last_watched_at, Some(700));
        assert_eq!(
    ...
    #[test]
    fn engagement_and_adoption_are_born_sync_clean() {
        let store = Store::open_memory().unwrap();
        identity_row(&store, 84);
        store.record_engagement(84, 500).unwrap();
        let show = store.get_show(84).unwrap().unwrap();
        assert_eq!(show.synced_status, Some(ListStatus::Planning));
        assert_eq!(show.synced_progress, Some(0));
        assert!(
            store.list_dirty_for_sync().unwrap().is_empty(),
            "engagement must never push to AniList"
        );
        // A real writer dirties it properly (after_play flips to watching).
        store.record_play(84, 1, true, 600).unwrap();
        assert_eq!(store.list_dirty_for_sync().unwrap().len(), 1);
        // Seeding is transition-only: re-engagement must not re-seed the
        // snapshot and silently mark the dirty row clean.
        store.record_engagement(84, 700).unwrap();
        assert_eq!(store.list_dirty_for_sync().unwrap().len(), 1);
        let show = store.get_show(84).unwrap().unwrap();
        assert_eq!(show.synced_status, Some(ListStatus::Planning));

        // Adoption path: same born-clean guarantee.
        let store2 = Store::open_memory().unwrap();
        identity_row(&store2, 85);
        store2
            .save_progress(85, Translation::Sub, "1", 100.0, 1400.0, None, 500)
            .unwrap();
        assert_eq!(store2.adopt_orphaned_progress().unwrap(), 1);
        assert!(store2.list_dirty_for_sync().unwrap().is_empty());
    }
    ...
    fn remote(id: i64, status: ListStatus, progress: u32) -> RemoteEntry {
        remote_at(id, status, progress, 0)
    }

    /// A remote entry with an explicit server edit time, for the duplicate
    /// collapse (06 §5.4).
    fn remote_at(id: i64, status: ListStatus, progress: u32, updated_at: i64) -> RemoteEntry {
        RemoteEntry {
            updated_at,
            anilist_id: id,
            status,
    ...
    #[test]
    fn a_downward_pull_keeps_a_live_resume_point() {
        // The stamp retires partials the frontier has passed. A correction moves
        // the frontier BEHIND the partial, so it must not stamp: the user is
        // mid-episode and the marker is what gets them back there.
        let store = Store::open_memory().unwrap();
        lib_row(
            &store,
            96,
        ...
        store
            .save_progress(96, Translation::Sub, "5", 600.0, 1400.0, None, 150)
            .unwrap();
        assert!(
            store.latest_resume(96, Translation::Sub).unwrap().is_some(),
            "partial is live before the pull"
        );
        store
            .reconcile_pull(&[remote(96, ListStatus::Watching, 3)], 200)
            .unwrap();
        assert_eq!(state(&store, 96).1, 3, "correction landed");
        assert!(
            store.latest_resume(96, Translation::Sub).unwrap().is_some(),
            "a downward frontier must not retire the partial ahead of it"
        );
        // A rise past the partial still retires it: the stamp keeps its meaning.
        store
            .reconcile_pull(&[remote(96, ListStatus::Watching, 9)], 300)
            .unwrap();
        assert!(store.latest_resume(96, Translation::Sub).unwrap().is_none());
    }

    #[test]
    fn duplicate_remote_entries_collapse_to_the_freshest_not_the_largest() {
        // AniList emits one entry per custom list a show is tagged in. Collapsing
        // by magnitude pins the stale copy and silently defeats every correction,
        // which is the smaller number by definition.
        let corrected = remote_at(95, ListStatus::Watching, 4, 900);
        let stale_duplicate = remote_at(95, ListStatus::Watching, 14, 100);
        // Order-independent: the fold compares stamps, not arrival.
        for payload in [
            vec![corrected.clone(), stale_duplicate.clone()],
            vec![stale_duplicate, corrected],
        ] {
            let store = Store::open_memory().unwrap();
            lib_row(
                &store,
            ...
            let out = store.reconcile_pull(&payload, 0).unwrap();
            assert_eq!(out.reconciled, 1);
            assert_eq!(state(&store, 95).1, 4);
            assert!(store.list_dirty_for_sync().unwrap().is_empty());
        }
    }
    ...
    #[test]
    fn pull_collapses_duplicate_remote_ids_keeping_the_freshest() {
        let store = Store::open_memory().unwrap();
        lib_row(
            &store,
            5,
        ...
        // Same id twice (custom-list duplication); the later edit wins.
        let out = store
            .reconcile_pull(
                &[
                    remote_at(5, ListStatus::Watching, 2, 100),
                    remote_at(5, ListStatus::Watching, 7, 200),
                ],
                0,
            )
            .unwrap();
