...
const CONTRACTS: &[Contract] = &[
    ...
    c("4", "Browse/Discover P adds a planning entry, never a provider-only row", Seeded("tui::app::browse_p_saves_the_highlighted_result")),
    c("4", "action-sync arms debounce on status/undo when connected (ROD-291)", Pending("M2 sync rail (ROD-448)")),
    // 5 · Layout gates
    c("5", "too-small terminal renders a degraded frame, never exits", Seeded("tui::app::tiny_terminal_renders_the_degraded_frame")),
    c("5", "pane_split_min 60; list width ~38% with 30-col floor", Seeded("tui::app::narrow_terminal_clamps_pane_to_list")),
    c("5", "top bar degrades with width", Seeded("tui::app::top_bar_degrades_with_width")),
    // 6 · View switching
    c("6", "F-keys/letters switch views; H is goto not toggle; search letters inert", Seeded("tui::app::switching_views_resets_pane_focus")),
    c("6", "dirty Settings leave via B/H/D/F-keys persists on the way out", Seeded("tui::app::ctrl_c_skips_a_dirty_settings_persist_and_fkey_leave_saves")),
    // 7 · Discover
    c("7", "entering Discover lands the feed; axis keys; error renders persistent", Seeded("tui::app::entering_discover_fetches_and_lands_the_feed")),
    c("7", "P on a card saves planning; chip tracks selection; / jumps to search", Seeded("tui::app::discover_p_saves_the_selected_card_as_planning")),
    c("7", "rows persist canonically (AniList id), never as provider bindings", Seeded("store::catalog_upsert_round_trips")),
    c("7", "multi-axis fan-out drain balance; retention cap exhaustion", Pending("M2 discover feed depth (ROD-449)")),
    c("7", "cover pump caps new fetches; prefetch next page near end", Pending("M2 discover feed depth (ROD-449)")),
    // 8 · Enrichment refresh (ROD-278)
    c("8", "enrich overwrites drift, preserves user state; never mints a row", Seeded("store::enrichment_patch_never_touches_user_state")),
    c("8", "confirmed no-match stamps freshness; stale total clears only on airing answer", Seeded("store::stale_total_clears_only_on_stamped_airing_answer")),
    ...
    // 12 · Covers
    c("12", "decision table: none/fetch/clear/up_to_date/suppress cooldown", Seeded("tui::view::detail::matching_cover_installs")),
    ...
    c("15", "provider caption: serving leads, markers, dim, shed order vs Pinned", Seeded("tui::view::detail::provider_row_composes_with_and_without_pin")),
    c("15", "Browse preview hides stale episode grid from History (T-9)", Seeded("tui::app::history_pane_entry_engages_and_list_focus_hides_the_grid")),
    // 16 · Search / command input
    c("16", "search chars append + debounce; letters do not navigate", Seeded("tui::app::typed_search_debounces_fetches_and_renders")),
    c("16", "load-more fires on Down arrow at the last result (ROD-156 parity)", Seeded("tui::app::browse_nav_pushes_the_shared_detail")),
...
const LEDGER: &[BugCheck] = &[
    b("ID-1", "FIX-IN-RUST: anilist_id SOT, bindings are edges", Seeded("store::bind_mints_identity_row_without_membership")),
    b("ID-2", "FIX-IN-RUST: durable catalog_cache, no library pollution", Seeded("store::enrichment_patch_never_mints_a_row")),
    b("ID-3", "CLONE: atomic migrate ladder + busy timeout", Seeded("store::concurrent_openers_serialize_on_the_ladder")),
    b("ID-4", "CLONE: propagate bind errors, never a silent NULL", Deferred("store bind-error injection deferred with the 439 store-failure harness")),
    ...
    b("A-4", "CLONE: CAS / contended skip on mid-edit sync", Pending("M2 sync rail (ROD-448)")),
    b("A-5", "CLONE (must-not): reloadAuth never frees a token mid-flush", Pending("M2 sync rail (ROD-448)")),
    b("STORE-FAIL", "injected store write/open failure paths (deferred at 439 review)", Deferred("ROD-440 inherits: no fault-injection seam yet; own it in an M2 store-hardening slice")),
];
...
fn every_05_section_is_present() {
    for n in 1..=17 {
        ...
        let present = CONTRACTS.iter().any(|c| {
            c.section == prefix || c.section.starts_with(&format!("{prefix}."))
        });
...
fn coverage_report() {
    ...
    for e in LEDGER {
        if let Pending(why) | Deferred(why) = e.cov {
            let tag = if matches!(e.cov, Pending(_)) { "PENDING" } else { "DEFER" };
...
fn still_airing_never_auto_completes_via_record_play() {
    ...
    store.add_to_library(&show(101, 12, "RELEASING"), 100).unwrap();
    ...
    assert_ne!(s.list_status, ListStatus::Completed, "airing never auto-completes");
}
...
fn finished_show_completes_at_total_via_record_play() {
    let store = Store::open_memory().unwrap();
    store.add_to_library(&show(102, 12, "FINISHED"), 100).unwrap();
    store.record_play(102, 12, true, 200).unwrap();
    assert_eq!(store.get_show(102).unwrap().unwrap().list_status, ListStatus::Completed);
}
...
fn partial_watch_records_play_without_ratcheting_progress() {
    let store = Store::open_memory().unwrap();
    store.add_to_library(&show(103, 12, "FINISHED"), 100).unwrap();
    store
...
fn completion_survives_a_lower_union_join() {
    let store = Store::open_memory().unwrap();
    store.add_to_library(&show(104, 12, "FINISHED"), 100).unwrap();
    store.set_list_status(104, ListStatus::Completed, 200).unwrap();
    assert_eq!(store.get_show(104).unwrap().unwrap().progress, 12);
    store
        .save_progress(104, Translation::Sub, "1", 100.0, 100.0, None, 300)
        .unwrap();
    assert_eq!(store.raise_progress_to_union(104, Translation::Sub).unwrap(), 12);
    let s = store.get_show(104).unwrap().unwrap();
    assert_eq!(s.progress, 12, "union of a single ep must not lower a completed show");
